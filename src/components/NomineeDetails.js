...
const NomineeDetails = ({ formData, nextStep, prevStep, updateFormData }) => {
  ...
  function findAndRemoveNomineeWithConfirmation(data, id) {
    ...
    if (nomineeExists && confirm(`Nominee ID "${id}" found. Do you want to remove this nominee from all allocations?`)) {
  ...
  const handleDelete = (index, remove) => {
    ...
    findAndRemoveNomineeWithConfirmation(formData, `nominee-${index}`);
    remove(index);
    const updatedNominees = formData.nomineeDetails.nominees.filter((_, i) => i !== index);
    updateFormData('nomineeDetails', { nominees: updatedNominees (updatedNominees.length ? updatedNominees : [emptyNominee]) });
  };
